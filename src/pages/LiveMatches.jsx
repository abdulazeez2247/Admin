  ...
  useEffect(() => {
    const fetchMatches = async () => {
      ...
      try {
        ...
        const sport = sportMap[selectedSportId] || "Unknown Sport";
        setSportName(sport);
      } catch (err) {
        console.error("Failed to fetch live matches:", err);
        setError("Failed to load live matches. Please try again later.");
      ...
      <main className="px-4 -mt-32">
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {matches.map((match) => (
                <div key={match.matchId} className="border-b border-gray-200 pb-4 last:border-b-0">
                  <p className="text-xl font-semibold text-gray-800">
                    {match.homeTeam} vs {match.awayTeam}
                  </p>
                  <p className="text-gray-700 text-sm">{match.competitionName}</p>
                  <p className="text-gray-600">
                    Time: {match.matchTime ? new Date(match.matchTime * 1000).toLocaleString() : 'N/A'}
                  </p>
                  <p className="text-gray-600">Status: {match.matchStatus}</p>
                  {match.streamUrl && (
                    <button
